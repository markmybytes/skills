...
After changing routes, regenerate types with `php artisan ziggy:generate --types-only`. Always pass `--types-only` — routes are injected at runtime via the `@routes` Blade directive and the `ziggy-js` package, so the generated `resources/js/ziggy.js` is not used and must not be committed. CI checks `resources/js/ziggy.d.ts` freshness.
